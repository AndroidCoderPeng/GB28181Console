...
public:
     ...
     * @param len
     */
    static void appendStartCode(std::vector<uint8_t> &dst, const uint8_t *data, size_t len);

    /**
     * 只要不加路数、不特殊缓冲策略，系统头就是固定值
     * */
    static std::vector<uint8_t> buildSystemHeader();

    /**
     * PSM除了CRC-32之外的其他各个字节在内容（流类型、ID、长度、头部）确定的情况下，都是固定不变的
     */
    static std::vector<uint8_t> buildPsm();

...
SipRegister::SipRegister(const std::string& local_host, const std::string& server_host, const int server_port,
                         ...
                         const std::string& device_name, const std::string& password, const double longitude,
                         const double latitude)
    : _local_host(local_host), _server_host(server_host), _server_port(server_port), _server_code(server_code),
      _server_domain(server_domain), _device_code(device_code), _serial_number(serial_number),
      _device_name(device_name), _password(password), _longitude(longitude), _latitude(latitude)
{
    auto* context = eXosip_malloc();
    if (!context)
    {
    ...
    // 监听本地端口
    result = eXosip_listen_addr(context, IPPROTO_UDP, nullptr, DEFAULT_SIP_PORT, AF_INET, 0);
    if (result != OSIP_SUCCESS)
    {
        std::cerr << "Listen local port " << DEFAULT_SIP_PORT << " failed: " << result << std::endl;
        eXosip_quit(_ex_context_ptr); // Quit and free context
...
void SipRegister::sip_event_loop()
{
    ...
    {
        auto* event = eXosip_event_wait(_ex_context_ptr, 100, 0);
        if (event == nullptr)
        {
            // 定期处理eXosip内部任务
    ...
    std::cout << "SIP Event Loop Thread Exited." << std::endl;
}

void SipRegister::setSipEventCallback(const SipEventCallback& callback)
{
    if (!_ex_context_ptr)
    {
    ...
    if (_reg_id > 0 && _reg_state.load() != REG_STATE_IDLE)
    {
        _reg_id = -1;
    }

    osip_message_t* reg = nullptr;
    const int reg_id = eXosip_register_build_initial_register(_ex_context_ptr, client, registrar,
                                                              nullptr, REGISTER_EXPIRED_TIME, &reg);
    if (reg_id < 0 || reg == nullptr)
    {
        _sip_event_callback(4011, "构建初始注册请求失败");
        return;
    }
    _reg_id = reg_id;
    _reg_state.store(REG_STATE_SENT_INITIAL);
    _is_do_unregister = false;

    const int result = eXosip_register_send_register(_ex_context_ptr, _reg_id, reg);
    if (result != OSIP_SUCCESS)
    {
        _sip_event_callback(4012, "发送初始注册请求失败");
        _reg_state.store(REG_STATE_FAILED);
        osip_message_free(reg); // 清理失败的消息
        return;
    }
    std::cout << "Initial registration sent." << std::endl;
}
...
void SipRegister::register_success()
{
    if (_is_do_unregister.load())
    {
        ...
        _reg_state.store(REG_STATE_IDLE);
    ...
    else
    {
        ...
        _reg_state.store(REG_STATE_SUCCESS);
...
void SipRegister::heartbeat()
{
    ...
    {
        for (int i = 0; i < HEARTBEAT_INTERVAL_SECONDS * 10 && _is_heartbeat_thread_running.load(); ++i)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }

        if (!_is_heartbeat_thread_running.load() || !_ex_context_ptr || _reg_id <= 0)
        {
            break; // Exit loop if stopped or context invalid
        }
        ...
        if (result != OSIP_SUCCESS || msg == nullptr)
        {
            std::cerr << "Failed to build heartbeat MESSAGE request." << std::endl;
...
void SipRegister::register_failed(const osip_message_t* response)
{
    if (response == nullptr)
    {
        _sip_event_callback(500, "未知错误 (响应为空)");
        _reg_state.store(REG_STATE_FAILED);
        return;
    }

    const int status_code = response->status_code;
    if (status_code == 401 && _reg_state.load() == REG_STATE_SENT_INITIAL)
    {
        eXosip_add_authentication_info(_ex_context_ptr, _device_name.c_str(), _device_code.c_str(), _password.c_str(),
                                       "MD5", nullptr);
        ...
        if (result != OSIP_SUCCESS || auth_reg == nullptr)
        {
            _sip_event_callback(4013, "构建认证注册请求失败");
            _reg_state.store(REG_STATE_FAILED);
            if (auth_reg) osip_message_free(auth_reg); // 清理失败的消息
            return;
        }
        ...
        if (result != OSIP_SUCCESS)
        {
            ...
            _reg_state.store(REG_STATE_FAILED);
            osip_message_free(auth_reg); // 清理失败的消息
            return;
        }
        _reg_state.store(REG_STATE_SENT_AUTH);
        std::cout << "Authentication registration sent." << std::endl;
    }
    else
    {
        _sip_event_callback(status_code, "注册失败");
        _reg_state.store(REG_STATE_FAILED);
    }
}
...
{
    if (event->request == nullptr)
    {
        std::cerr << "platform_event: Received event with null request." << std::endl;
        return;
    }
    ...
    if (eXosip_message_build_answer(_ex_context_ptr, event->tid, 200, &ack) == OSIP_SUCCESS && ack != nullptr)
    {
        eXosip_message_send_answer(_ex_context_ptr, event->tid, 200, ack);
    }
    else
    {
        std::cerr << "Failed to build/send 200 OK for MESSAGE." << std::endl;
    }

    // 解析 Content-Type
    const osip_content_type_t* ct = osip_message_get_content_type(event->request);
    if (!ct || osip_strcasecmp(ct->type, "Application") != 0 || osip_strcasecmp(ct->subtype, "MANSCDP+xml") != 0)
    {
        std::cerr << "Unsupported content type: "
            << (ct ? ct->type : "NULL") << "/" << (ct ? ct->subtype : "NULL")
            << std::endl;
        return;
    }
    ...
    {
        std::cerr << "platform_event: Body is empty or null." << std::endl;
        return;
    }
    ...
    if (content.empty())
    {
        std::cerr << "platform_event: Content string is empty." << std::endl;
        return;
    }
    ...
    if (!xml_result)
    {
        std::cerr << "platform_event: Error parsing XML: " << xml_result.description() << std::endl;
        return;
    }
    ...
    if (!query_node)
    {
        std::cerr << "platform_event: No Query node found in XML." << std::endl;
        return;
    }
    ...
    if (result != OSIP_SUCCESS || msg == nullptr)
    {
        std::cerr << "platform_event: Failed to build response MESSAGE." << std::endl;
        return;
    }
...
void SipRegister::start_push_stream(const eXosip_event_t* event)
{
    if (event->request == nullptr)
    {
        std::cerr << "start_push_stream: Received event with null request." << std::endl;
        eXosip_call_send_answer(_ex_context_ptr, event->tid, 500, nullptr); // Send error response
        return;
    }
    ...
    if (local_port < 0)
    {
        ...
        std::cerr << "Failed to initialize RTP socket after building 200 OK" << std::endl;
        osip_message_free(answer); // Clean up the answer message we built
        return;
    }
    ...
    if (ret != OSIP_SUCCESS)
    {
        _sip_event_callback(402, "发送200 OK响应失败，初始化推流失败");
        return;
    }
...
void SipRegister::deregistration()
{
    if ((_reg_id <= 0 && _reg_state.load() == REG_STATE_IDLE) || !_ex_context_ptr || !_is_sip_loop_running.load())
    {
        std::cout << "Already idle or not initialized, skipping deregistration." << std::endl;
        return;
    }
    ...
    if (result != OSIP_SUCCESS || un_reg == nullptr)
    {
        std::cerr << "Build unregister request failed: " << result << std::endl;
        _is_do_unregister = false; // Reset flag if build fails
        return;
    }
    ...
    if (result != OSIP_SUCCESS)
    {
        ...
        osip_message_free(un_reg); // Clean up failed message if send fails
        _is_do_unregister = false; // Reset flag
        return;
    }

    _reg_id = -1;
    std::cout << "Unregistration request sent." << std::endl;
}

SipRegister::~SipRegister()
{
    std::cout << "SipRegister destructor called." << std::endl;

...
bool VideoFrameEncoder::prepare() {
    // 查找H.264编码器
    ...
    _av_codec_ctx_ptr->width = FRAME_WIDTH;
    _av_codec_ctx_ptr->height = FRAME_HEIGHT;
    _av_codec_ctx_ptr->bit_rate = 4000000; // 720P: 2-3 Mbps, 1080P: 3-6 Mbps
    _av_codec_ctx_ptr->time_base = {1, 90000}; // 90kHz时间基
...
void VideoFrameEncoder::encodeFrame(const std::shared_ptr<cv::Mat> &mat_ptr) const {
    ...
    // 创建SWS上下文进行格式转换
    SwsContext *sws_ctx_ptr = sws_getContext(
        mat.cols, mat.rows, AV_PIX_FMT_BGR24,
        _av_codec_ctx_ptr->width, _av_codec_ctx_ptr->height, _av_codec_ctx_ptr->pix_fmt,
    ...
    sws_scale(sws_ctx_ptr, src_slice,
              src_stride, 0, mat.rows, yuv_frame_ptr->data,
              yuv_frame_ptr->linesize);
...
void VideoFrameEncoder::stopEncode() const {
    std::lock_guard<std::mutex> lock(_encoder_mutex);
    // 刷新编码器
    if (_av_codec_ctx_ptr) {
...
VideoFrameEncoder::~VideoFrameEncoder() {
    ...
    PsMuxer::get()->release();
    std::cout << "VideoFrameEncoder 释放完成" << std::endl;
}